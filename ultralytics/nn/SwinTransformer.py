...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # print(attn.dtype, v.dtype) #详细改进流程和操作，请关注B站博主：Ai学术叫叫兽 er,畅享一对一指点迷津，已指导无数家人拿下学术硕果！！！
        try:
...
class SwinTransformerC(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        num_heads = c_ // 32
        self.m = SwinTransformerBlock(c_, c_, num_heads, n)
        # self.m = nn.Sequential(*[Bottleneck(c_, c_, shortcut, g, e=1. #详细改进流程和操作，请关注B站博主：Ai学术叫叫兽 er,畅享一对一指点迷津，已指导无数家人拿下学术硕果！！！0) for _ in range(n)])
...
def window_reverse(windows, window_size, H, W):
    ...
    x = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(B, H, W, -1) #详细改进流程和操作，请关注B站博主：Ai学术叫叫兽 er,畅享一对一指点迷津，已指导无数家人拿下学术硕果！！！
    return x
...
class SwinTransformerLayer(nn.Module):
    ...
    def create_mask(self, H, W):
        ...
        return attn_mask
    def forward(self, x):
        # reshape x[b c h w] to x[b l c] #详细改进流程和操作，请关注B站博主：Ai学术叫叫兽 er,畅享一对一指点迷津，已指导无数家人拿下学术硕果！！！
        _, _, H_, W_ = x.shape
        Padding = False
        ...
        return x #详细改进流程和操作，请关注B站博主：Ai学术叫叫兽 er,畅享一对一指点迷津，已指导无数家人拿下学术硕果！！！
