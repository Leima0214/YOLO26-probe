            ...
            if self.infer_mode:
                ...
                if dw_size > 1:
                    self.cheap_rpr_scale = self._conv_bn(
                        init_channels, new_channels, 1, 1, 0, groups=init_channels, bias=False
                    )
        ...
        if self.mode in ["ori"]:
            ...
            if self.gate_loc == "before":
                return out[:, : self.oup, :, :] * F.interpolate(
                    self.gate_fn(res / self.scale), size=out.shape[-2:], mode=self.inter_mode
                )  # 'nearest'
    ...
    def __init__(
        ...
        if layer_id <= 1:
            ...
            self.ghost1 = GhostModule(
                in_chs, mid_chs, relu=True, mode="ori_shortcut_mul_conv15", args=args
        ...
        for block_cfg in block_specs:
            ...
            for k, exp_size, c, se_ratio, s in block_cfg:
                ...
                if block == GhostBottleneck:
                    layers.append(
                        block(input_channel, hidden_channel, output_channel, k, s, se_ratio=se_ratio, layer_id=layer_id)
                    )
                input_channel = output_channel
                layer_id += 1
