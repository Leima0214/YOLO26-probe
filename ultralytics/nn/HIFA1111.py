...
class OutlookAttention(nn.Module):
    """Implementation of outlook attention --dim: hidden dim --num_heads: number of heads --kernel_size: kernel size in
    each window for outlook attention return: token features after outlook attention.
    """
...
if __name__ == "__main__":
    input = torch.randn(1, 1, 7, 7)  # 随机生成一个输入特征图
    se = OutlookAttention(dim=7, num_heads=7)  # 实例化SE模块，设置降维比率为8
