...
class ConvFFN(nn.Module):
    def __init__(self, in_channels, internal_channels, out_channels, drop_path):
        ...
        self.pw1 = conv_bn(
            in_channels=in_channels, out_channels=internal_channels, kernel_size=1, stride=1, padding=0, groups=1
        )
        self.pw2 = conv_bn(
            in_channels=internal_channels, out_channels=out_channels, kernel_size=1, stride=1, padding=0, groups=1
        )
...
class RepLKNet_Stem(nn.Module):
    def __init__(self, in_channel, channels):
        ...
        self.stem = nn.ModuleList(
            [
                conv_bn_relu(
                    in_channels=in_channel, out_channels=base_width, kernel_size=3, stride=2, padding=1, groups=1
                ),
                conv_bn_relu(
                ...
                conv_bn_relu(
                    in_channels=base_width, out_channels=base_width, kernel_size=1, stride=1, padding=0, groups=1
                ),
                conv_bn_relu(
                    in_channels=base_width,
                    out_channels=base_width,
...
class RepLKNet_stage1(nn.Module):
    def __init__(
        ...
        self.transition = nn.Sequential(
            ...
            conv_bn_relu(channels[0 + 1], channels[0 + 1], 3, stride=2, padding=1, groups=channels[0 + 1]),
        )
...
class RepLKNet_stage2(nn.Module):
    def __init__(
        ...
        self.transition = nn.Sequential(
            ...
            conv_bn_relu(channels[1 + 1], channels[1 + 1], 3, stride=2, padding=1, groups=channels[1 + 1]),
        )
...
class RepLKNet_stage3(nn.Module):
    def __init__(
        ...
        self.transition = nn.Sequential(
            ...
            conv_bn_relu(channels[2 + 1], channels[2 + 1], 3, stride=2, padding=1, groups=channels[2 + 1]),
        )
