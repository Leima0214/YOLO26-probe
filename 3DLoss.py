...
def plot_total_loss_3d_simple(csv_path, save_path=None, start_epoch=0, end_epoch=None, figsize=(14, 10)):
    ...
    total_loss_df, _loss_components = calculate_total_loss(csv_path)
    ...
    scatter1 = ax1.scatter(epochs, np.zeros_like(epochs), loss_values, c=loss_values, cmap="viridis", s=30, alpha=0.7)
    ...
    for i, (epoch, loss) in enumerate(zip(epochs, loss_values)):
        ax2.bar3d(epoch, 0, 0, dx, dy, loss, color=colors[i], alpha=0.7, edgecolor="black", linewidth=0.3)
    ...
    plt.cm.RdYlGn((loss_diff - loss_diff.min()) / (loss_diff.max() - loss_diff.min() + 1e-8))

    scatter4 = ax4.scatter(epochs, loss_diff, loss_values, c=loss_values, cmap="coolwarm", s=50, alpha=0.8)
    ...
    plt.suptitle("YOLO Total Loss 3D Visualization (Combined Loss)", fontsize=16, fontweight="bold", y=0.95)
    plt.tight_layout()
...
def plot_total_loss_3d_alternative(csv_path, save_path=None, start_epoch=0, end_epoch=None, figsize=(14, 10)):
    ...
    for i, (xi, yi, loss) in enumerate(zip(x, y, loss_values)):
        ...
        ax1.bar3d(xi, yi, z_bottom[i], dx, dy, loss, color=color, alpha=0.8, edgecolor="black", linewidth=0.3)
    ...
    ax2.plot(np.zeros_like(epochs), np.zeros_like(epochs), epochs, "r-", linewidth=2, label="Center Line")
    ...
    plt.suptitle("Alternative 3D Visualizations of Total Loss", fontsize=16, fontweight="bold", y=0.95)
    plt.tight_layout()
...
def plot_total_loss_2d_with_3d_effect(csv_path, save_path=None, start_epoch=0, end_epoch=None, figsize=(16, 12)):
    ...
    ax3 = axes[0, 2]
    ax3.scatter(epochs, loss_values, c=loss_values, cmap="viridis", s=50, alpha=0.8, edgecolors="black", linewidth=0.5)
    ax3.plot(epochs, loss_values, "b-", alpha=0.3)

    for i in range(len(epochs) - 1):
        ax3.fill_between([epochs[i], epochs[i + 1]], [loss_values[i], loss_values[i + 1]], 0, alpha=0.1, color="blue")
    ...
    if len(epochs) > 10:
        ...
        ax4.contourf(X, Y, Z, levels=20, cmap="plasma", alpha=0.8)
    ...
    if len(loss_components) > 0:
        ...
        for i, col in enumerate(loss_components[:3]):
            if col in total_loss_df.columns:
                ...
                data = data[mask]
                ax5.fill_between(epochs, bottom, bottom + data, alpha=0.5, label=f"Loss {i + 1}")
                bottom += data
    ...
    plt.suptitle("YOLO Total Loss Analysis with 3D Effects", fontsize=16, fontweight="bold", y=0.98)
    plt.tight_layout()
    ...
    if not Path(csv_file_path).exists():
        ...
        try:
            ...
            fig1, data1 = plot_total_loss_3d_simple(
                csv_path=csv_file_path, save_path="yolo_total_loss", start_epoch=0, end_epoch=None, figsize=(14, 10)
            )

            print("\n2. 绘制替代3D图...")
            fig2 = plot_total_loss_3d_alternative(
                csv_path=csv_file_path, save_path="yolo_total_loss", start_epoch=0, end_epoch=100, figsize=(14, 10)
            )

            print("\n3. 绘制2D带3D效果图...")
            fig3 = plot_total_loss_2d_with_3d_effect(
                csv_path=csv_file_path, save_path="yolo_total_loss", start_epoch=0, end_epoch=None, figsize=(16, 12)
            )
